...
class SetFrameRange(Application):
    ...
    def run_app(self):
        ...
        (new_head_in, new_in, new_out, new_tail_out) = self.get_frame_range_from_shotgun()
        ...
        updated = self.set_frame_range(self.engine.name, new_head_in, new_in, new_out, new_tail_out)
    ...
    def get_frame_range_from_shotgun(self):
        ...
        sg_head_in_field = self.get_setting("sg_head_in_frame_field")
        sg_in_field = self.get_setting("sg_in_frame_field")
        sg_out_field = self.get_setting("sg_out_frame_field")
        sg_tail_out_field = self.get_setting("sg_tail_out_frame_field")

        fields = [sg_head_in_field, sg_in_field, sg_out_field, sg_tail_out_field]

        data = self.shotgun.find_one(sg_entity_type, filters=sg_filters, fields=fields)

        # check if fields exist!
        if sg_head_in_field not in data:
            raise tank.TankError("Configuration error: Your current context is connected to a Shotgun "
                                 ...
                                 "field %s.%s!" % (sg_entity_type, sg_entity_type, sg_head_in_field))
        ...
        if sg_tail_out_field not in data:
            raise tank.TankError("Configuration error: Your current context is connected to a Shotgun "
                                 "%s. This entity type does not have a "
                                 "field %s.%s!" % (sg_entity_type, sg_entity_type, sg_tail_out_field))

        return ( data[sg_head_in_field], data[sg_in_field], data[sg_out_field], data[sg_tail_out_field] )
